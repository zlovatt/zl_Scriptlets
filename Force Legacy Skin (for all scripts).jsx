(function forceLegacySkinForAllScripts () {
	...
	var closeOpenScripts = false;			// Quickly close & open _EVERY INSTALLED SCRIPT_ to update
	var logSuccess = false;					// Write success/fail to ESTK console for each
	...
	function getScriptUIFiles () {
		...
		if (isWindows) {
			var userScriptUIPath = checkAlias(Folder.userData).fullName + "/Adobe/After Effects/" + aeVersion + "/Scripts/ScriptUI Panels";
			var userScriptUIFolder = checkAlias(new Folder(userScriptUIPath));
			if (userScriptUIFolder.exists)
				scriptFiles = scriptFiles.concat (userScriptUIFolder.getFiles(searchTerm));
		}
